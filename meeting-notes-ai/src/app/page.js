  ...
  return (
     <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      ...
      {uploadedFiles.length > 0 && (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-3 border-b border-white/10 bg-black/10">
          ...
          <div className="flex flex-wrap gap-2">
            {uploadedFiles.map((file) => (
              <div
                key={file.id}
                className="flex items-center space-x-2 px-2 sm:px-3 py-1 bg-purple-600/20 border border-purple-500/30 rounded-lg text-xs sm:text-sm"
              >
                <span className="text-purple-200 truncate max-w-32 sm:max-w-none">{file.name}</span>
      ...
      {/* Main Content */}
      <div className="max-w-4xl mx-auto flex flex-col h-[calc(100vh-120px)] sm:h-[calc(100vh-140px)]">
        {/* Messages Area */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
          {messages.length === 0 ? (
            <div className="text-center py-8 sm:py-12">
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-purple-500 to-pink-500 rounded-2xl mx-auto mb-4 flex items-center justify-center">
                      ...
                      <div className="mt-2 sm:mt-3 pt-2 border-t border-white/20">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {msg.attachedFiles.map((file, idx) => (
                            ...
                            >
                              <span className="truncate max-w-24 sm:max-w-none">{file.name}</span>
        ...
        {summary && showSummaryEditor && (
          <div className="border-t border-white/10 bg-black/20 backdrop-blur-sm p-4 sm:p-6">
            ...
            <textarea
              ...
              rows={4}
              className="w-full p-3 sm:p-4 bg-white/10 border border-white/20 rounded-lg resize-none focus:outline-none focus:border-purple-500 transition-colors text-sm"
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
              <div className="flex items-center space-x-3 sm:space-x-2">
                <label className="flex items-center space-x-2 px-3 sm:px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg cursor-pointer transition-colors text-sm font-medium">
                ...
                {uploadedFiles.length > 0 && (
                  <button
                    ...
                    disabled={isLoading}
                    className="flex items-center space-x-1 sm:space-x-2 px-3 sm:px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 rounded-lg transition-colors text-sm font-medium"
                  >
              ...
              <div className="flex space-x-2 sm:space-x-3 flex-1">
                <input
                  ...
                  className="flex-1 px-3 sm:px-4 py-2 sm:py-3 bg-white/10 border border-white/20 rounded-lg focus:outline-none focus:border-purple-500 transition-colors placeholder-gray-400 text-sm"
                  disabled={isLoading}
                />
                <button
                  onClick={sendMessage}
                  disabled={isLoading || !currentInput.trim()}
                  className="px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg transition-colors flex items-center space-x-1 sm:space-x-2 text-sm"
                >
            ...
            {/* Email Section */}
            {summary && (
              <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 pt-3 sm:pt-4 border-t border-white/10">
                <input
                  type="email"
                  placeholder="Enter recipient email..."
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="flex-1 px-3 sm:px-4 py-2 bg-white/10 border border-white/20 rounded-lg focus:outline-none focus:border-emerald-500 transition-colors placeholder-gray-400 text-sm"
                />
                <div className="flex space-x-2 sm:space-x-3">
                  <button
                    onClick={() => setShowSummaryEditor(!showSummaryEditor)}
                    className="px-3 sm:px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors flex items-center space-x-1 sm:space-x-2 text-sm"
                  >
                    <Edit3 className="w-4 h-4" />
                    <span>Edit</span>
                  </button>
                  <button
                    ...
                    className="px-3 sm:px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg transition-colors flex items-center space-x-1 sm:space-x-2 text-sm"
                  >
